...
func bfisflag(b, s string) bool {
	if (b == "(cap," || b == "(up," || b == "low,") && s[1] == ' ' {
		return false
	}
	...
	for i := 0; i < len(b); i++ {
		for j := 0; j < len(b[i]); j++ {
			...
			if strings.Contains(b[i][j], "(up,") {
				...
				j--
				// fmt.Println(b[i][j-1])
			} else if strings.Contains(b[i][j], "(low,")  {
				if j == 0 {
					b[i][j] = ""
				...
				j--
				// fmt.Println(b[i][j-1])
			} else if strings.Contains(b[i][j], "(cap,") {
				if j == 0 {
					b[i][j] = ""
